...
#define SERVERB_PORT "31871"	// the port users will be connecting to
...
{
	int sockfd;
	struct addrinfo hints, *servBinfo, *mainservinfo, *p1, *p2;
	int rv;
	int numbytes;
	char buf[MAXBUFLEN];
	socklen_t addr_len;
	...
	for(p2 = servBinfo; p2 != NULL; p2 = p2->ai_next) {
	...
	cout << "Server B is up and running using UDP on port " << SERVERB_PORT << endl;
	...
	cout << "Server B has sent a country list to Main Server" << endl;
    ...
    while(1) {
		...
		cout << "Server B has received a request for finding possible friends of User " << user_id << " in " << country << endl;
		...
		if (iter != country_friend_map.end()) {
			...
			if (friend_set.size() > 1) {
                ...
                cout << "Server B found the following possible friends for User " << user_id << " in " << country << ": " << send_msg << endl;;			
                ...
                cout << "Server B found the following possible friends for User " << user_id << " in " << country << ": None" << endl;;			
				cout << "Server B has sent the result to Main Server" << endl;
			}			
			...
			cout << "Server B has sent \"User " << user_id << " not found\" to Main Server" << endl;
		}
    }
